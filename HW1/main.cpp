...
int getMove(const struct board &place); /* Gets a legal user move each turn */
int registerMove(struct board &place, int player, int column);
/* Registers moves to the board according to given column. WARNING!: This function doesnt check if given column is
 * legal or not. It depends on other functions which gets moves to check their validity. It just check full or not */
...
int main() {
    ...
    coords aiCalcCoords{}; /* Coordinates of ai move lines */

    bool isItWin = false; /* Draw or Win? */
    bool endGame = false; /* Main switch for game loop */
    ...
    while (!endGame) {
        ...
        if (aiEnable && whichPlayer == 10)
            registerMove(gameBoard, whichPlayer, playerMoveCol = getAiMove(gameBoard, aiCalcCoords)); /* Computer Move */
        else
            while (registerMove(gameBoard, whichPlayer, playerMoveCol = getMove(gameBoard))) /* Player Move */
        ...
        whichPlayer = 1 == whichPlayer ? aiEnable ? 10 : 15 : 1; /* AI is 10, Second Player is 15 and First Player is 1 */
        numOfMoves++;
...
int getMove(const struct board &place) {
    char move = 0;
    bool isInputValid = false;
    char lastCol = 'A';
